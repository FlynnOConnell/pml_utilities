"""Selection conversion shared by the GUI, lbm-suite2p-python, isoview.

One entry point that turns a per-axis selection (the kind of value the GUI
selection table and CLI produce) into 0-based index lists keyed by the axis
letter ``T``/``C``/``Z``, plus thin adapters to the keyword names each
consumer expects. Keeps the 1-based-public / 0-based-internal rule and the
view/cam/plane/timepoint aliasing in one place.

- ``selection_to_indices(arr, {"View": "1:2", "Z": "1:14"})`` ->
  ``{"C": [0, 1], "Z": [0..13]}`` (0-based).
- ``to_lsp_kwargs`` emits 1-based ``timepoints``/``planes``/``channels`` (the
  selection keywords lbm-suite2p-python and imwrite share).
...
def axis_sizes(arr) -> dict[str, int]:
...
def selection_to_indices(arr, selections, one_based: bool = True) -> dict[str, list[int]]:
    ...
    """
    sizes = axis_sizes(arr)
    out: dict[str, list[int]] = {}
    for key, sel in selections.items():
        letter = _axis_letter(key)
        if letter is None or sel is None:
            continue
        out[letter] = parse_selection(sel, sizes[letter], one_based=one_based)
    return out


def to_lsp_kwargs(indices: dict[str, list[int]]) -> dict[str, list[int]]:
    """0-based indices per axis letter -> lbm-suite2p-python / imwrite 1-based kwargs.

    Emits ``timepoints``/``planes``/``channels`` (1-based, the shared
    selection keywords). Only present axes are emitted.
    """
    out: dict[str, list[int]] = {}
    if "T" in indices:
        out["timepoints"] = [i + 1 for i in indices["T"]]
    if "Z" in indices:
        out["planes"] = [i + 1 for i in indices["Z"]]
    if "C" in indices:
        out["channels"] = [i + 1 for i in indices["C"]]
    return out


def to_isoview_kwargs(indices: dict[str, list[int]]) -> dict[str, list[int]]:
    """0-based indices per axis letter -> isoview pipeline kwargs (0-based).
    ...
    out: dict[str, list[int]] = {}
    if "T" in indices:
        out["timepoints"] = list(indices["T"])
    if "C" in indices:
        out["cameras"] = list(indices["C"])
    return out
