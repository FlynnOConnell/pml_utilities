...
- the max projection over depth of every Z-stack whose field holds the
  scan's lines or patches.
...
class ReferenceImage:
    """One image of the popup: what the combo calls it, its pixels, and the
...
def reference_images(mesc, open_unit: Callable[[str], object], c: int) -> list[ReferenceImage]:
    ...
    projection over its frames and depth in channel ``c``, carrying the
    unit's ROIs. Empty for a unit with no ROIs, or none placed on an
    image of the file. ``open_unit`` opens a sibling unit by key; the
    ...
    for ref_key in refs:
        ...
        chunk = max(1, min(nz, MAX_ELEMENTS // max(-(-nt // step) * ny * nx, 1)))
        peak = np.full((ny, nx), -np.inf, np.float32)
        for k0 in range(0, nz, chunk):
            block = np.asarray(ref[::step, cc, k0 : k0 + chunk], np.float32)
            np.maximum(peak, block.reshape(-1, ny, nx).max(axis=0), out=peak)
        name = ref_key.rsplit("/", 1)[-1]
        out.append(ReferenceImage(f"{name} {'Z-stack max' if nz > 1 else 'picture'}", peak, records))
    return out
...
class ReferenceView:
    ...
    def open(self, mesc, open_unit: Callable[[str], object], select: str | None = None) -> bool:
        """Show the popup for ``mesc``, on image ``select`` when it is one of
        the popup's; False when no image of the file carries its ROIs."""
        ...
        if not self.images:
            return False
        if select in self.images:
            self.viewer.set_images({k: im.image for k, im in self.images.items()}, selected=select)
        self.viewer.open()
        return True
    ...
    def _toolbar(self, viewer) -> None:
        """The popup's caption: how many of the unit's ROIs are on this image
        and which one is thick."""
        ...
        imgui.text_disabled(f"{self.unit}: {len(im.records)} {what} drawn here, ROI {roi + 1} thick")
        set_tooltip("Click a line or patch to move the ROI slider to it.", show_mark=False)
