...
import numpy as np
from imgui_bundle import hello_imgui, imgui, imgui_ctx, portable_file_dialogs as pfd
...
_IMPORTANT_FIELDS = {"thres_type", "soft_levels", "thres_bp_sd", "thres_amp_sd", "sigma_dfof"}
...
def _is_default(obj, name: str) -> bool:
    ...
    if isinstance(cur, (tuple, list)) and isinstance(default, (tuple, list)):
        return len(cur) == len(default) and all(
            math.isclose(float(a), float(b), rel_tol=1e-6, abs_tol=1e-9) for a, b in zip(cur, default, strict=True)
        )
    return cur == default
...
class VoltagePipelineWidget(PipelineWidget):
    ...
    def _ensure_state(self) -> None:
        ...
        if pf is not None:
            ...
            try:
                ...
                domains, scan_ids, first_env = spec["domains"], spec["scan_ids"], spec["first_env"]
    ...
    def _frame_window(self) -> tuple[int, int] | None:
        ...
        idx = list(getattr(parsed, "final_indices", []) or []) if parsed is not None else []
    ...
    def draw_config(self) -> None:
        self._ensure_state()
        with imgui_ctx.push_style_var(imgui.StyleVar_.item_spacing, imgui.ImVec2(8, 4)), fit_width():
            imgui.spacing()
            ...
            imgui.spacing()
            if imgui.button("Pipeline Settings##voltage_settings", imgui.ImVec2(hello_imgui.em_size(11), 0)):
                self._show_settings_popup = True
            set_tooltip("dF/F, denoiser and peak parameters. Defaults are the archive's, so an untouched run reproduces its PF folders.")
    ...
    def _draw_slice_row(self) -> None:
        ...
        if self._show_slice_popup:
            ...
            self._show_slice_popup = False
        imgui.set_next_window_size(imgui.ImVec2(hello_imgui.em_size(36), 0), imgui.Cond_.first_use_ever)
    ...
    def _draw_scans_block(self) -> None:
        ...
        for u in self._units:
            ...
            if self._scans[key]:
                imgui.same_line()
                _, self._first_env[key] = imgui.checkbox(f"env##voltage_env_{key}", self._first_env.get(key, False))
                if imgui.is_item_hovered():
    ...
    def _draw_domains_block(self) -> None:
        ...
        if imgui.is_item_hovered():
            imgui.set_tooltip(f"Save the table, scans and env flags to {DOMAINS_FILE} beside the file.")
    ...
    def _load_domains_file(self) -> None:
        ...
        try:
            ...
            return
        self._domain_rows = [[n, ",".join(str(r) for r in rois)] for n, rois in spec["domains"].items()
    ...
    def _save_domains_file(self) -> None:
        ...
        mesc = self._mesc_path()
        path = (mesc.parent if mesc else Path.home()) / DOMAINS_FILE
        scans = [u["key"].rsplit("_", 1)[-1] for u in self._units if self._scans.get(u["key"])]
        first_env = [u["key"].rsplit("_", 1)[-1] for u in self._units
                     if self._scans.get(u["key"]) and self._first_env.get(u["key"])]
        doc = {"mesc": mesc.name if mesc else "", "scans": scans, "first_env": first_env, "domains": domains}
    ...
    def _f_float(self, obj, field: str, label: str, tooltip: str, step=0.1, fmt="%.2f") -> None:
        pushed = self._mod_push(obj, field)
        imgui.set_next_item_width(hello_imgui.em_size(6))
        _, val = imgui.input_float(f"##voltage_{field}", float(getattr(obj, field)), step, step * 10, fmt)
        if pushed:
            imgui.pop_style_color()
    ...
    def _f_check(self, obj, field: str, label: str, tooltip: str) -> None:
        pushed = self._mod_push(obj, field)
        val = checkbox_with_tooltip(f"{label}##voltage_{field}", bool(getattr(obj, field)), tooltip)
        if pushed:
            imgui.pop_style_color()
    ...
    def _draw_settings_popup(self) -> None:
        ...
        imgui.set_next_window_size_constraints(
            imgui.ImVec2(min(hello_imgui.em_size(62), viewport.size.x * 0.9), hello_imgui.em_size(12)),
            imgui.ImVec2(viewport.size.x * 0.98, viewport.size.y * 0.98),
        )
        opened, visible = imgui.begin_popup_modal(
            popup_title, p_open=True, flags=self._settings_sizer.flags(imgui.WindowFlags_.no_saved_settings),
        )
        if not opened:
            return
        try:
            ...
            with imgui_ctx.push_style_var(imgui.StyleVar_.item_spacing, imgui.ImVec2(8, 4)):
                ...
                set_tooltip("Reset every parameter to the archive's settings (Noguchi & Terada).", show_mark=False)
                imgui.same_line()
                imgui.set_cursor_pos_x(imgui.get_window_width() - btn_w - imgui.get_style().window_padding.x)
                ...
                if imgui.button("Close##voltage_settings_close", imgui.ImVec2(btn_w, 0)):
                    imgui.close_current_popup()
                imgui.pop_style_color(3)
        finally:
            imgui.end_popup()
    ...
    def _draw_settings_columns(self) -> None:
        ...
        col_w = max((avail_w - 2 * imgui.get_style().item_spacing.x) / 3, hello_imgui.em_size(18))
        for i, (title, key, body) in enumerate(columns):
            if i:
                imgui.same_line()
            imgui.begin_child(f"##voltage_col_{key}", imgui.ImVec2(col_w, 0),
    ...
    def _draw_dfof_params(self) -> None:
        ...
        self._f_float(d, "sigma_dfof", "dF/F baseline sigma",
                      "Gaussian baseline under dF/F, in samples (1500 = 1.4 s at 1075 Hz).", step=100, fmt="%.0f")
        self._f_float(d, "sigma_baseline", "z baseline sigma",
                      "Slower Gaussian baseline removed before the z-score, in samples.", step=100, fmt="%.0f")
        self._f_int(d, "n_startup", "Start-up samples", "Leading samples replaced by the mean of the rest (filter start-up).")
        self._f_check(d, "negative", "Flip sign", "JEDI darkens on depolarisation; on makes spikes positive.")
    ...
    def _draw_denoiser_params(self) -> None:
        ...
        self._row_tail(den, "thres_type", "Mask type", "Soft keeps a fraction of every band outside events; the archive used soft.")
        pushed = self._mod_push(den, "soft_levels")
        imgui.set_next_item_width(hello_imgui.em_size(12))
        _, levels = imgui.input_float4("##voltage_soft_levels", list(den.soft_levels), "%.2f")
        if pushed:
            imgui.pop_style_color()
        den.soft_levels = tuple(float(v) for v in levels)
        self._row_tail(den, "soft_levels", "Soft levels", "Attenuation outside events for bands below 5, 5-30, 30-80 and above 80 Hz.")
        self._f_float(den, "hard_floor", "Hard floor", "Attenuation outside events in hard mode.", step=0.01)
        self._f_check(den, "complex_bands", "Complex bands above 75 Hz", "As the original code; off takes the real part first.")
        self._f_int(den, "n_scales", "Scales", "Wavelet scales, log-spaced between the two below.", lo=8)
        self._f_float(den, "scale_min", "Scale min", "Smallest wavelet scale in samples.", step=0.5, fmt="%.1f")
        self._f_float(den, "scale_max", "Scale max", "Largest wavelet scale in samples.", step=50, fmt="%.0f")
        self._f_float(den, "lp_cutoff_hz", "Baseline cutoff Hz",
                      "FIR low-pass of the z-score saved as lp_FIR1Hz; not added to the curated trace.", step=0.5, fmt="%.1f")
        self._f_float(den, "fir_window_ms", "FIR window ms", "Hamming window length of the baseline filter.", step=100, fmt="%.0f")
        self._f_check(den, "fir_odd_taps", "Odd FIR taps", "Trim the tap count to odd. Off reproduces the archive.")
        imgui.set_next_item_open(False, imgui.Cond_.appearing)
        if imgui.collapsing_header("Clustering and windows##voltage_adv"):
            self._f_int(den, "n_components", "PCA components", "PCA components of the wavelet rows.", lo=2)
            self._f_int(den, "n_comp_clu", "Components clustered", "Leading components the clustering uses.", lo=1)
            self._f_int(den, "n_clusters", "Clusters", "Coarse cut of the frequency tree (unused downstream).", lo=1)
            self._f_int(den, "n_subclusters", "Bands", "Frequency bands reconstructed.", lo=1)
            self._f_float(den, "slow_upthres", "Open level, slow (SD)", "Event window opens at this many SD for bands below 30 Hz.")
            self._f_float(den, "fast_upthres", "Open level, fast (SD)", "Event window opens at this many SD for bands at or above 30 Hz.")
    ...
    def _draw_event_params(self) -> None:
        ...
        try:
            self._f_float(ev, "bp_low", "Band-pass low Hz", "Low edge of the Butterworth band-pass.", step=1, fmt="%.0f")
            self._f_float(ev, "bp_high", "Band-pass high Hz", "High edge of the Butterworth band-pass.", step=10, fmt="%.0f")
            self._f_float(ev, "thres_bp_sd", "Band-passed SD", "Peaks of the band-passed trace above mean + this many SD.")
            self._f_float(ev, "thres_amp_sd", "Amplitude SD", "Peak height on the trace itself above mean + this many SD.")
            self._f_float(ev, "duration_thres_ms", "Min duration ms", "Shortest stretch above the on/off level a peak may sit on.", step=1, fmt="%.1f")
            self._f_int(ev, "distance_samples", "Min spacing", "Samples between peaks; 3 reproduces the archive.", lo=1)
        finally:
            if not ev.detect:
                imgui.end_disabled()
    ...
    def _draw_modified_table(self) -> None:
        mods = []
        for prefix, obj in (("dfof", self.settings.dfof), ("denoiser", self.settings.denoiser),
                            ("events", self.settings.events), ("runtime", self.settings.runtime)):
            for f in dataclasses.fields(obj):
                if not _is_default(obj, f.name):
                    mods.append((f"{prefix}.{f.name}", getattr(obj, f.name), _field_default(obj, f.name)))
        imgui.text(f"Modified parameters ({len(mods)})")
        set_tooltip("Every parameter that differs from the archive's value, with the default beside it.")
        if not mods:
            imgui.text_disabled("All parameters at the archive's defaults")
            return
        mod_h = min(hello_imgui.em_size(10), imgui.get_frame_height_with_spacing() * (len(mods) + 2))
        if imgui.begin_child("##voltage_mod_params", imgui.ImVec2(-1, mod_h), imgui.ChildFlags_.borders):
            flags = imgui.TableFlags_.row_bg | imgui.TableFlags_.borders_inner_h | imgui.TableFlags_.sizing_stretch_prop
            if imgui.begin_table("##voltage_mod_tbl", 3, flags):
                ...
                for field, cur, default in mods:
                    ...
                    imgui.table_set_column_index(2)
                    imgui.text_disabled(f"{default:.3g}" if isinstance(default, float) else str(default))
                imgui.end_table()
        imgui.end_child()
    ...
    def _draw_run(self) -> None:
        ...
        if imgui.is_item_hovered(imgui.HoveredFlags_.allow_when_disabled):
            imgui.set_tooltip(reason or f"Spawn a worker for {len(scans)} scan(s); the process console tracks it.")
    ...
    def _submit(self, mesc: Path, scans: list[str], domains: dict, window, planes=None) -> None:
        ...
        args = {
            "input_path": str(mesc),
            "reader_kwargs": widget_reader_kwargs(getattr(self.parent, "image_widget", None)),
